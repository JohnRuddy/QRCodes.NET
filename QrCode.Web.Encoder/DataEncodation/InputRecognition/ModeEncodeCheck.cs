...
namespace QrCode.Web.Encoder.DataEncodation.InputRecognition
{
    ...
    public static class ModeEncodeCheck
    {
        ...
        /// <summary>
        /// Determines whether [is mode encode valid] [the specified mode].
        /// </summary>
        /// <param name="mode">The mode.</param>
        /// <param name="encoding">The encoding.</param>
        /// <param name="content">The content.</param>
        /// <returns><c>true</c> if [is mode encode valid] [the specified mode]; otherwise, <c>false</c>.</returns>
        /// <remarks></remarks>
        public static bool isModeEncodeValid(Mode mode, string encoding, string content)
        {
            switch (mode)
            {
                ...
                default:
                    throw new InvalidOperationException(string.Format("System does not contain mode: {0}",
                                                                      mode.ToString()));
            }
        }
        ...
        internal static int TryEncodeKanji(string content, int contentLength)
        {
            ...
            for (int index = 0; index < contentLength; index++)
            {
                ...
                if (length != 2)
                    return index == 0 ? 0 : -2;
                else if ((mostSignificantByte < 0x81 || mostSignificantByte > 0x9F) &&
                         (mostSignificantByte < 0xE0 || mostSignificantByte > 0xEB))
                {
                    return index == 0 ? 0 : -2;
                }
            }
