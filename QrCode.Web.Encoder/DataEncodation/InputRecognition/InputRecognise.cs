...
namespace QrCode.Web.Encoder.DataEncodation.InputRecognition
{
    ...
    public static class InputRecognise
    {
        ...
        /// <summary>
        /// Eights the bit byte recognision.
        /// </summary>
        ...
        private static string EightBitByteRecognision(string content, int startPos, int contentLength)
        {
            ...
            scanPos = ModeEncodeCheck.TryEncodeEightBitByte(content, QRCodeConstantVariable.DefaultEncoding, scanPos,
                                                            contentLength);
            if (scanPos == -1)
                return QRCodeConstantVariable.DefaultEncoding;

            foreach (var kvp in eciSet)
            {
